...
mv t $FILEHD
rm -rf initial_error
/home/teliu/Apps/sketchmap-master/bin/dimred -vv -D $HD -d $LD -w -preopt 20 -grid $GW,20,200 -gopt 5 < $FILEHD > tmp 2>>initial_error
...
for ((ITER=1; ITER<=$MAXITER; ITER++)); do
   ...
   if [ ! -e $FILELD.gmds_$ITER ]; then
      ...
      grep -v \#  $FILEHD | /home/teliu/Apps/sketchmap-master/bin/dimred -vv -D $HD -d $LD -w -preopt 50 -grid 100,20,200 -fun-hd $SIGMAHD,$AHD,$BHD -fun-ld $SIGMALD,$ALD,$BLD -init tmp -gopt 5 -imix $IMIX < $FILEHD > $FILELD.gmds_$ITER 2>>log
   fi
...
((ITER--))
grep -v "#" $FILELD.gmds_$ITER | awk '{print $1, $2}' > tmp
grep -v \#  $FILEHD | /home/teliu/Apps/sketchmap-master/bin/dimred -vv -D $HD -d $LD -w -preopt 50 -grid $GW,20,200 -fun-hd $SIGMAHD,$AHD,$BHD -fun-ld $SIGMALD,$ALD,$BLD -init tmp -gopt 5 > $FILELD.gmds 2>>log
